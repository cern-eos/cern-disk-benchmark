...
if(_mount STREQUAL "" )
  if(CMAKE_ARGC GREATER 1)
    # Collect positional args, skipping any that look like a .cmake script path.
    set(_pos_args "")
    math(EXPR _last "${CMAKE_ARGC} - 1")
    foreach(i RANGE 1 ${_last})
      set(val "${CMAKE_ARGV${i}}")
      if(val MATCHES "\\.cmake$")
        continue()
      endif()
      list(APPEND _pos_args "${val}")
    endforeach()

    list(LENGTH _pos_args _alen)
    if(_alen GREATER 0)
      list(GET _pos_args 0 _mount)
    endif()
    if(_alen GREATER 1)
      list(GET _pos_args 1 _parallel)
    endif()
    if(_alen GREATER 2)
      list(GET _pos_args 2 _stop)
    endif()
  endif()
endif()
